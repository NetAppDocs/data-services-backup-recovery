...
NOTE: You cannot unprotect an application while protection operations are still running for it. As a workaround, <<Remove a restore point for a Kubernetes application, remove the restore point>> the running protection operation is using. You can then unprotect the application.
