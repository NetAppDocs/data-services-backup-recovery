...
NOTE: You cannot unprotect an application while protection operations are still running for it. Either wait for the operation to finish, or as a workaround, <<Remove a restore point for a Kubernetes application, remove the restore point>> the running protection operation is using. You can then unprotect the application.
...
. Choose the recovery point you need to delete and select the Actions icon image:../media/icon-action.png[Actions option] > *Delete recovery point* to delete it.