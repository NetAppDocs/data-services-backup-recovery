...
== Create a protection group using tags

Use tag-based protection to simplify administration of protection groups in Backup and Recovery. 

Here's how tag-based protection works:

* When you add a tag to a VM or datastore in vCenter, that object is automatically included in the next backup for any protection group that references that tag.
* When you remove a tag from a VM or datastore, that object is automatically excluded from subsequent backups for that tag-based protection group.
* If a tag is deleted in vCenter, future backups skip that tag and typically report a warning.
...
. In the resource type selection step, select *Tags*.

. If multiple vCenters are configured, select the appropriate vCenter from the list to show tags available for that vCenter.

. In the tag selection interface:
.. In the left panel, review the table of tags, which includes:
+
* *Tag Name*
* *Category*
* Optional VM and datastore counts
+
.. Select the check boxes for one or more tags.
.. In the right panel, review the list of selected tags, grouped by vCenter.
.. Remove any tag you do not want in the protection group.

. Configure the backup policy and schedule:
.. Select an existing backup policy (for example, *Daily-Backup*).
.. Configure the backup schedule, retention, and backup tier (local and/or cloud) as required.

. In the *Review* step:
.. Confirm that the resource type for the protection group is *Tags*.
.. Verify the set of tags that will be protected.
.. If you are editing an existing tag-based protection group, review:
+
* *Existing Tags*: Unchanged tags
* *Added Tags*: Newly added tags
* *Removed Tags*: Tags that will be removed from the group
+
...
+
The new tag-based protection group appears in the Protection Groups list, indicated by a tag icon.
