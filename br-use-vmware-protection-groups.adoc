...
* Create a protection group by manually adding VMs to the group or by using VMware tags.
...
* When you add a tag to a VM or datastore in vCenter, that object is automatically included in the next backup for any Backup and Recovery protection group that references that tag.
...
* If a tag is deleted in vCenter, future backups skip that tag and report a warning.
