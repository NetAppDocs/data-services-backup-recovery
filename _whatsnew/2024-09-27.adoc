...
Console agent version 3.9.40 supports certain Red Hat Enterprise Linux 8 and 9 releases for manual Console agent installations on RHEL hosts, in addition to the operating systems listed in the https://docs.netapp.com/us-en/console-setup-admin/task-prepare-private-mode.html#step-3-review-host-requirements[host requirements^]. These newer RHEL versions require the Podman engine instead of the Docker engine. Previously, NetApp Backup and Recovery had two limitations when using the Podman engine; those limitations have been removed.
