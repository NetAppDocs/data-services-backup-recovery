...
For Kubernetes clusters running Trident Protect 26.04 or later, you can upgrade Trident Protect directly from NetApp Backup and Recovery.

.Steps
. In NetApp Backup and Recovery, select *Inventory*.
. Select *Kubernetes* from the list at the top right of the page.
. Choose a Kubernetes instance, and select the Actions icon image:../media/icon-action.png[Actions option] > *Upgrade cluster*.
