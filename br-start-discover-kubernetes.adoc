...
* When the cluster is discovered and added to Backup and Recovery, the backup schedule used with Trident Protect is no longer active. You need to protect the cluster's applications by creating a new protection policy or associating the applications with an existing policy.
=====
