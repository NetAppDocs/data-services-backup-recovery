...
[NOTE]
=====
If you are discovering an existing cluster that is protected with Trident Protect:

* Backup and Recovery detects clusters that are managed with Trident Protect and displays slightly different instructions during the following steps.
* When the cluster is discovered and added to Backup and Recovery, the backup schedule used with Trident Protect is no longer active. You need to protect the cluster's applications by link:br-use-policies-create.html[creating a new protection policy] or associating the applications with an existing policy.
=====
