...
. Optionally, select a scope at which to apply access control:
+
* *Folder*: Select this option to control user access to resources at the folder level, and select a folder from the list.
* *Project*: Select this option to control user access to resources at the project level, and select a project from the list.
+
...
* Select *Air-gapped* to discover clusters that have no external network connectivity. The instructions are tailored to air-gapped clusters.
+
[role="tabbed-block"]
====
.Connected discovery
