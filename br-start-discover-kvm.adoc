...
[.lead]
Discover KVM hosts and virtual machines in NetApp Backup and Recovery so you can protect them with protection policies. Your KVM environment must be managed by a management platform such as Apache CloudStack before discovery.
...
* Select *Done*.
... Add a new KVM host by entering the following information:
+
* *KVM FQDN or IP address*: Enter the host's FQDN or IP address.
* *Credentials*: Select a stored credential for the KVM host.
* *Agents*: Choose the Console agent to use for communication between Backup and Recovery and the KVM host.
.. *Advanced settings*: Optionally, enter a port to communicate with the KVM host and select the host's management platform from the list. 
. Select *Discover*. 
+
