...
When installing a new Console agent, download the same software version as the original agent. Changes to the NetApp Backup and Recovery database can cause newer software versions to not work with old database backups. You can https://docs.netapp.com/us-en/console-setup-admin/task-upgrade-connector.html[upgrade the Console agent software to the most current version after restoring the Backup database^].
...
. Log in to NetApp Console using the admin user credentials that you just created.

=== Restore the NetApp Backup and Recovery database

. Copy the MySQL backup from the backup location to the new Console agent host. The following example uses the file name `CBS_DB_Backup_23_05_2023.sql`.
...
. Copy the Indexed Catalog backup ZIP file from the backup location to the `/opt/application/netapp/cbs` folder on the new Console agent host. The following example uses the file name `Indexed_Catalog_DB_Backup_catalogdb1_23_05_2023.zip`.
...
. Run the *ls* command to make sure that the folder "catalogdb1" has been created and that it contains the subfolders `files`, `versions`, and `snapshots`.
...
Add the details of the StorageGRID system that is associated with your ONTAP systems, as they were configured in the original Console agent setup, by using the https://docs.netapp.com/us-en/console-automation/index.html[NetApp Console APIs^].
...
----
+
While the IP address, username, and password values are custom values, the account name is not. The account name is always `account-DARKSITE1`. Also, the username must use an email-formatted name.
+
This API returns a response like the following. You can retrieve the authorization token as shown below.
+
[source,text]
----
...
----
+
This API returns a response like the following. The value under `resourceIdentifier` denotes the _WorkingEnvironment ID_, and the value under `agentId` denotes _x-agent-id_.
+
[source,text]
----
...
. Update the NetApp Backup and Recovery database with the details of the StorageGRID system that is associated with the systems. Make sure that you enter the fully qualified domain name of StorageGRID, along with the access key and storage key, as shown below:
+
[source,shell]
----
...
. Select each ONTAP system and select *View Backups* next to the Backup and Recovery service in the right panel.
+
You should see all backups created for your volumes.

. From the Restore Dashboard, under the Search & Restore section, select *Indexing Settings*.
+
Make sure that the systems that had Indexed Cataloging enabled previously remain enabled.
