...
You can also follow these steps to replace the Console agent and restore configuration data when using Backup and Recovery in _standard mode_ on-premises when no cloud accounts are involved (for example, with a backup destination of NetApp StorageGrid).
