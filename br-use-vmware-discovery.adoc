...
[.lead]
Before you can protect VMware datastores and VMs running on ONTAP systems, you need to discover them in NetApp Backup and Recovery. If you already have SnapCenter Plug-in for VMware vSphere installed, you can also import its backup data and policies.
...
If you don't already have SnapCenter Plug-in for VMware installed, add vCenter information so NetApp Backup and Recovery can discover workloads. Within each Console agent, select the systems where you want to discover workloads. 
...
.. *vCenter settings*: Add a new vCenter. To add a new vCenter, enter the vCenter FQDN or IP address, select a Console agent, and select credentials to use to authenticate with vCenter.
+
...
If you already have SnapCenter Plug-in for VMware installed, import SnapCenter Plug-in resources into NetApp Backup and Recovery using these steps. The Console discovers ESXi hosts, datastores, VMs, and schedules from the Plug-in, so you do not have to recreate that information. 
