...
[.lead]
In NetApp Backup and Recovery, you can back up volume data from your on-premises ONTAP systems to a secondary storage system and to Azure Blob. Use this page to prepare your environment and activate backups.
...
** For NetApp Backup and Recovery PAYGO licensing, you'll need a subscription to the https://marketplace.microsoft.com/en-us/product/netapp.cloud-manager?tab=Overview[NetApp Console offering from the Azure Marketplace^]. Billing for NetApp Backup and Recovery is done through this subscription.
...
+
Note that NetApp Backup and Recovery supports _Azure access policies_ as the permission model. The _Azure role-based access control_ (Azure RBAC) permission model is not currently supported.
...
+
NOTE: If you select *Bring your own bucket*, ensure that the container you create as the backup destination has the same name as the Azure storage account.
