...
= Create and manage Hyper-V backup policies in NetApp Backup and Recovery
:hardbreaks:
:icons: font
:imagesdir: ./media/

[.lead]
Use NetApp Backup and Recovery to create Hyper-V backup policies that control backup frequency, schedule, and retention.
...
NOTE: For Hyper-V workloads, backup schedules use the time zone of the Hyper-V host, not UTC or your browser's local time. This time zone is set when you create the schedule. If the host time zone changes, you must refresh the host and re-create the schedule for the new time zone to take effect. Refer to link:br-use-hyperv-protection-groups.html[Create and manage protection groups for Hyper-V workloads] for more information.
...
* *Enable integrity scan*: Select whether you want to enable integrity scans (snapshot locking) on the object storage. This ensures backups are valid and restorable. The integrity scan frequency is set to seven days by default. To protect your backups from being modified or deleted, select the *Integrity scan* option. The scan occurs only on the latest snapshot. You can enable or disable integrity scans on the latest snapshot. 
...
====
* *VM Settings*:
** *Enable application-consistent snapshot*: Enable this option to create application-consistent snapshots. This requires both Hyper-V Integration Services and VSS to be running on the VM. If either service is not running, a crash-consistent backup is taken instead. Enabling application consistency might increase both backup time and storage usage. Additionally, the VM's guest memory (RAM) is not included in application-consistent snapshots.
...
* *Ransomware scan*: Select whether you want to enable ransomware scanning on each bucket. This requires DataLock locking on object storage. Enter the frequency of the scan in days. This option applies to object storage. This option might incur additional charges, depending on the cloud provider.
...
If you imported resources from SnapCenter, you might encounter some differences between policies used in SnapCenter and those used in NetApp Backup and Recovery. See link:reference-policy-differences-snapcenter.html[Policy differences between SnapCenter and NetApp Backup and Recovery].
