...
+
* *VM Settings*:
** *Enable application-consistent snapshot*: Enable this option to create application-consistent snapshots. This requires both Hyper-V Integration Services and VSS to be running on the VM. If either service is not running, a crash-consistent backup is taken instead. Note that enabling application consistency may increase both backup time and storage usage. Additionally, the VM's guest memory (RAM) is not included in application-consistent snapshots.
...
* *Ransomware scan*: Select whether you want to enable ransomware scanning on each bucket. This requires DataLock locking on object storage. Enter the frequency of the scan in days. This option applies to object storage. Note that this option might incur additional charges, depending on the cloud provider.
