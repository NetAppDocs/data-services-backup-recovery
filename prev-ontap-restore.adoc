...
* You can restore a *folder* from replicated or object storage snapshots to a volume in the original system, to a volume in a different system that's using the same cloud account, or to a volume on an on-premises ONTAP system. Restoring folders from local snapshots is not supported.
