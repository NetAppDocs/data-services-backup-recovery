...
[NOTE]
=====
Restoring a volume or file from archive storage can take additional time. The job remains *In progress* in the Job Monitor while Backup and Recovery retries the restore based on the selected priority.
During this period, you might see the following message for the volume restore job in the *Audit* timeline:
...
This means archive retrieval is in progress. Monitor the restore in the *Job Monitor*; it completes when the job reaches a final status, such as *Completed* or *Failed*, after all retry attempts. Don't start another restore unless the job ultimately fails.
=====
