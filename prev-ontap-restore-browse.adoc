...
[.lead]
With NetApp Backup and Recovery, restore ONTAP data using Browse & Restore. Before restoring, note the source volume name, source system and storage VM, and backup file date. You can restore ONTAP data from a snapshot, a replicated volume, or from backups stored in object storage.
