...
summary: With NetApp Backup and Recovery, restore ONTAP data using Browse and Restore.
---

= Restore ONTAP data using Browse & Restore in NetApp Backup and Recovery
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
[.lead]
Use Browse & Restore when you know the source volume name, source system, storage VM, and backup file date. You can restore ONTAP data from a snapshot, a replicated volume, or a backup stored in object storage.
...
[NOTE]
=====
When you restore a volume or file from a backup that is stored in archive storage, the restore might require additional time before the data is available. The restore job remains In progress in the Job Monitor while Backup and Recovery automatically retries the operation according to the selected restore priority.
During this period, the *Volume/File restore* action in the *Audit* timeline might temporarily display the following message:
`Objects are being restored from archive storage. Retry the operation after the time interval specific to the restore priority level.`
This message indicates that the archive retrieval is still in progress. Monitor the restore status in the *Job Monitor*. The restore is complete when the job reaches a final status, such as *Completed* or *Failed* after all retry attempts are made. Avoid starting another restore unless the job ultimately fails.
=====
...
NOTE: The High restore priority isn't supported when restoring data from Azure archival storage to StorageGRID systems.

== Browse & Restore supported systems and object storage providers

You can restore ONTAP data from a backup file that resides in a secondary system (a replicated volume) or in object storage (a backup file) to the following systems. 
...
//ifdef::aws[]
* For Amazon S3, the Console agent can be deployed in AWS or on premises
//endif::aws[]
//ifdef::azure[]
* For Azure Blob, the Console agent can be deployed in Azure or on premises
//endif::azure[]
//ifdef::gcp[]
* For Google Cloud Storage, the Console agent must be deployed in your Google Cloud Platform VPC
//endif::gcp[]
* For StorageGRID, the Console agent must be deployed on premises, with or without internet access
* For ONTAP S3, the Console agent can be deployed on premises (with or without internet access) or in a cloud provider environment

Note that references to "on-premises ONTAP systems" include FAS, AFF, and ONTAP Select systems.
...
NOTE: When you restore a SnapLock-enabled volume, the restored volume retains SnapLock protection only if the destination storage is WORM-enabled. If you restore to a non-WORM destination, the SnapLock status is removed from the restored volume.
...
NOTE: Quick restore is supported for FlexGroup volumes only if the source system from which the cloud backup was created was running ONTAP 9.12.1 or greater. It is supported for SnapLock volumes only if the source system was running ONTAP 9.11.0 or greater.
...
. Select the *Restore* tab to display the Restore Dashboard.
...
+
//ifdef::aws[]
* When restoring from Amazon S3, select the IPspace in the ONTAP cluster where the destination volume will reside, enter the access key and secret key for the user you created to give the ONTAP cluster access to the S3 bucket, and optionally choose a private VPC endpoint for secure data transfer. If you use a private VPC endpoint, it takes precedence over any configured proxy connection.
//endif::aws[]
//ifdef::azure[]
...
. Optionally, enable *Prioritize this restore over existing job queue* to give this restore operation higher priority. When enabled, other running jobs might be paused or delayed to allow this restore to proceed.
. Select *Restore*.
+
You return to the Restore Dashboard so you can review the progress of the restore operation.
...
link:prev-reference-gcp-archive-storage-tiers.html[Learn more about restoring from Google archival storage]. Backup files in the Google Archive storage tier are restored almost immediately, and require no Restore Priority.
+
If ransomware protection is active for the backup file (if you enabled DataLock and Ransomware protection in the backup policy), you are prompted to run an additional ransomware scan on the backup file before restoring the data. We recommend that you scan the backup file for ransomware. (You'll incur extra egress costs from your cloud provider to access the contents of the backup file.)
...
+
You have a few options for the location when restoring folders and files.
+
* When you have chosen *Select Target Folder*, as shown above:
+
** You can select any folder.
** You can hover over a folder and select the end of the row to drill down into subfolders, and then select a folder.
+
* If you selected the same destination system and volume as the source folder or file location, you can select *Maintain Source Folder Path* to restore the folder or files to the same folder where they existed in the source structure. All the same folders and subfolders must already exist; folders are not created. When restoring files to their original location, you can choose to overwrite the source files or create new files.
