...
= Back up Cloud Volumes ONTAP to Google Cloud Storage with NetApp Backup and Recovery
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
[.lead]
Use NetApp Backup and Recovery to back up volume data from your Cloud Volumes ONTAP systems to Google Cloud Storage.
...
You also need a Google Cloud account for the storage space where your backups will be located.
...
To use the NetApp Backup and Recovery Search & Restore functionality, the role for the Console agent must include specific permissions so it can access the Google Cloud BigQuery service. See the permissions below, and follow the steps if you need to modify the policy.
...
.. Select *Edit Project* and select a new project if the one you want to use is different from the default project (where the Console agent resides).
...
. *Services*: Leave NetApp Backup and Recovery enabled and select *Continue*.
...
+
If the Google Cloud Storage destination for your backups exists as a system on the Console *Systems* page, you can drag the ONTAP cluster onto the Google Cloud Storage system.
...
+ 
If you choose to use your own customer-managed keys, enter the key ring and key information.
