...
=== Host mapping 
SnapCenter does not have map storage clusters or storage VMs for the resources to hosts, but NetApp Backup and Recovery does. The on-premises ONTAP cluster or storage VM will not be mapped to a host in NetApp Backup and Recovery preview versions. Additionally, NetApp Console does not support storage VMs. 
