...
= Known limitations with NetApp Backup and Recovery for SQL Server workloads
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
[.lead]
Review these limitations before using NetApp Backup and Recovery for SQL Server workloads.
...
You cannot assign multiple policies that use the same schedule tier (for example, hourly, daily, or weekly) to a resource. NetApp Backup and Recovery will not import those resources from SnapCenter.
...
If a SnapCenter policy repeats in both day and month intervals, NetApp Backup and Recovery will not import the policy.

For example, you cannot attach a daily policy (with an interval of 7 days or less, or more than 7 days) to a resource and also attach a monthly policy to the same resource.
...
=== Host mapping 
SnapCenter does not map storage clusters or storage VMs for resources to hosts, but NetApp Backup and Recovery does. The on-premises ONTAP cluster or storage VM will not be mapped to a host in NetApp Backup and Recovery preview versions. Additionally, NetApp Console does not support storage VMs.
