...
[.lead]
Use NetApp Backup and Recovery to manage snapshot, replication, and object-storage backups for your Cloud Volumes ONTAP and on-premises ONTAP systems. You can change schedules, enable or disable volume backups, pause backups, delete backups, and unregister the service.
...
. Review the status of the protection options in the "Existing protection" column. The three icons stand for "Local snapshots," "Replicated volumes," and "Backups in object storage."
+
...
Refer to link:br-start-enable.html[Enable NetApp Backup and Recovery to protect volumes^] for instructions.
...
. From the *Volumes* menu,  locate the volume for which you want to modify the policy settings, select the Actions menu image:icon-action.png[Actions icon] at the end of the row, and select *Edit backup strategy*.
...
. Depending on the type of backup action you want to configure, select the button in the Bulk actions menu:
+ 
...
. Select image:icon-actions-horizontal.gif[Actions icon] for the volume backup file you want to scan for ransomware and select *Scan for Ransomware*.
+
...
NOTE: You can't delete individual volume backup files once the backup relationship has been deleted. Delete the backup files for a volume before you delete the volume's backup relationship.

.Steps

. From the *Volumes* menu, select image:icon-actions-horizontal.gif[More icon] for the source volume and select *Manage backups > View backup*.
+
The list of all backup files is displayed.
. Choose a backup from the list and select *Actions > Delete all Backups*.
. Confirm deletion by entering the volume name.
. Expand the *Advanced settings* area.

. Under *Force delete backups*, select whether to force-delete all backups.
+
In rare cases you might need to revoke NetApp Backup and Recovery's access to backups; for example, if Backup and Recovery loses access to the backup bucket or backups are DataLock-protected but you no longer want them. Use this option to force-delete backups at the volume and system level.
+
CAUTION: Use this option only for extreme cleanup. Once enabled, NetApp Backup and Recovery will no longer access these backups (even if the objects remain in cloud storage). You must manually delete them from your cloud provider.

. Select *Delete*.

=== Delete a single backup file for a volume

Delete an individual backup when it's no longer needed. You can't delete replicated volumes (data protection volumes).

NOTE: You can't delete individual volume backup files once the backup relationship has been deleted. You can, however, <<Delete all backup files for a volume,delete all backup files for the volume>>.
...
NOTE: You can't delete individual volume backup files once the backup relationship has been deleted. You can, however, <<Delete all backup files for a volume,delete all backup files for the volume>>. If you need to delete a backup relationship for a volume, make sure to delete the backups for the volume first, then delete the backup relationship.
...
Deactivating NetApp Backup and Recovery for a system pauses backups and restores for all its volumes without deleting existing backups. This enables you to pause all backup and restore activity for a period of time without unregistering the backup service from this system.
