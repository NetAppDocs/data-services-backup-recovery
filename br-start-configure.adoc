...
[.lead]
Configure Backup and Recovery settings to add credentials for workloads, integrate with existing infrastructure, and prepare backup and restore operations.

* <<Add credentials for host resources>> for Windows, Microsoft SQL Server, Oracle Database, and Linux hosts that NetApp Backup and Recovery needs to authenticate with. This includes guest OS credentials used when restoring files or folders.
...
* <<Create a resource transformation template>> to modify resources as you restore them.
...
+
TIP: For Microsoft SQL Server workloads, you must enter credentials for both Windows and Microsoft SQL Server, requiring two sets of credentials.
+
[role="tabbed-block"]
====
...
Provide VMware vCenter Server credentials to discover workloads for backup. If you don't have credentials, create them with permissions to access and manage VMware vCenter Server workloads.
...
* *vCenter FQDN or IP address*: Enter an FQDN or the IP address for the VMware vCenter Server.
...
If you use the Apache CloudStack management platform to manage KVM resources, integrate it with NetApp Backup and Recovery so that Backup and Recovery can discover and protect the managed KVM hosts and VMs.
...
. From the NetApp Backup and Recovery menu, select *Settings*.
. Select the down arrow to expand the *Management platform* section.
. Select *Add management platform credential*.
...
Before you create policies for Windows hosts, configure log directories in snapshots to store the logs generated during the backup process. 
...
Create resource transformation templates to store common transformations you apply to resources during restore operations. 
...
. From the NetApp Backup and Recovery menu, select *Settings*.
. Select the down arrow to expand the *Resource transformation template* section.
. Select *Create template*.
. Enter a name for the resource transformation template.
. Optionally, enter a template description.
. Describe the target resource:
.. Enter the resource kind (such as Pod or Deployment). This field is required.
.. Optionally, enter a resource group, version, and name.
. Build an operation:
.. Choose an operation from the *Operation* list.
.. Enter the JSON path that the operation applies to.
.. If applicable, enter a value to add or update, or a JSON path to move from.
+
These fields only appear for operations that require them.
.. To add another operation to this template, select *Add operation*.
. When finished, select *Add*.