...
* <<Add credentials for host resources>> for any Windows, Microsoft SQL Server, Oracle Database, or Linux hosts that NetApp Backup and Recovery needs to authenticate with. This includes guest OS credentials used when restoring files or folders.
...
If you use the Apache CloudStack management platform to manage KVM resources, you need to integrate it with NetApp Backup and Recovery so that Backup and Recovery can discover and protect the managed KVM hosts and VMs.
...
Create resource transformation templates to store common transformations you make to resources as you restore them. 

.Steps

. From the NetApp Backup and Recovery menu, select *Settings*.
. Select the down arrow to expand the *Resource modifer template* section.
...
.. Optionally, enter a resource group, version, name.
. Build an operation:
