...
= About NetApp Backup and Recovery
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
[.lead]
Use this overview to learn how NetApp Backup and Recovery protects your ONTAP workloads, what workloads and backup targets it supports, and how licensing and costs work.
...
Support for Backup and Recovery is already built into all ONTAP systems, so there is no need for additional hardware or media gateways. This makes backup operations simple and cost effective. NetApp Console simplifies any backup strategy implementation, including the full spectrum of 3-2-1 backup variants, without needing multiple resource managers or specialized personnel.
...
** Refer to link:br-use-oracle-protect-overview.html[Protect Oracle Database workloads].
...
//endif::azure[]
//ifdef::gcp[]
** In Google Cloud, a new bucket is deployed, and the https://cloud.google.com/bigquery[Google Cloud BigQuery services^] are provisioned on an account/project level.
//endif::gcp[]
...
* In Azure, NetApp Console uses a new or existing resource group with a storage account for the Blob container. The Console https://docs.microsoft.com/en-us/azure/storage/blobs/anonymous-read-access-prevent[blocks public access to your blob data] by default.
