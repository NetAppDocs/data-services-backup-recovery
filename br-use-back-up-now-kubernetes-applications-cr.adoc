...
summary: Use custom resources (CRs) to create on-demand snapshots and backups of a Kubernetes application, establishing a baseline or protecting your most recent data changes.
---

= Back up Kubernetes applications now using custom resources in NetApp Backup and Recovery
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
[.lead]
Use custom resources (CRs) to create on-demand snapshots and backups of a Kubernetes application, establishing a baseline or protecting your most recent data changes.
...
* *spec.reclaimPolicy*: (_Optional_) Defines what happens to the AppArchive of a snapshot when the Snapshot CR is deleted. Even when set to `Retain`, the Snapshot CR itself is still deleted; only the AppArchive is retained. Valid options:
** `Retain` (default)
** `Delete`
+
[source,yaml]
...
* *spec.dataMover*: (_Optional, required for clusters migrated from Trident Protect_) A string indicating which backup tool to use for the backup operation. If this cluster has been migrated to Backup and Recovery from Trident Protect, the value is case sensitive and must be `CBS`.
...
|string
|Maximum time (in seconds) to wait for any newly created PersistentVolumeClaims (PVCs) to reach the `Bound` phase before the operation fails.
|"1200" (20 minutes)
|===
