...
== View collected resources for a snapshot or backup using a custom resource
Use a ResourceViewer CR to see what resources have been collected as part of a completed snapshot or backup.

.Steps
. Create the view resources CR file:
.. Create the custom resource (CR) file and name it (for example, `view-resources-cr.yaml`).
...
* *metadata.ui-correlation-id*: (_Required_) 
