...
[.lead]
Restore KVM virtual machines from primary snapshots, replicated secondary copies, or object storage backups.
...
You can restore virtual machines from these locations: 
...
If you select a backup in object storage and ransomware protection is active for that backup (if you enabled DataLock and ransomware protection in the backup policy), you are prompted to run an additional integrity check before you restore the data. We recommend that you perform the scan. 
...
When you restore virtual machines, the following applies: 
...
From the Restore page, you can restore a VM even if you don't remember its exact name, location, or last known good date. You can search for the snapshot by using filters. 
...
Restore KVM virtual machines using the Restore menu. You can search for snapshots by name or by using filters. 
...
. Enter the name of the VM that you want to restore, or filter by the VM host or storage pool where the VM is located. 
+
A list of snapshots appears that match your search criteria.

. Select *Restore* for the snapshot that you want to restore. 
+
