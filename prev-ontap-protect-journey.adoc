...
= Plan ONTAP volume protection in NetApp Backup and Recovery
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
[.lead]
Use this page to plan how you'll protect your ONTAP volumes with NetApp Backup and Recovery. Review the protection methods, architectures, policies, object storage choices, and Console agent deployment modes before you enable backup and recovery.
...
Before you select the features you'll use, here's a quick explanation of what each feature does and the type of protection it provides.
...
In addition to these criteria, cloud storage offers additional security options if you use the *DataLock and Ransomware protection* feature, and additional cost savings when you select archival storage classes for older backup files. link:prev-ontap-policy-object-options.html[Learn more about DataLock and Ransomware protection and archival storage settings].
...
You can create snapshot, replication, and backup-to-object-storage policies in the NetApp Backup and Recovery UI. See link:prev-ontap-backup-manage.html[adding a new backup policy] for details.
...
Backup policies reside in different locations depending on whether you use a fan-out or cascade architecture. Replication policies and backup policies are designed differently because replication pairs two ONTAP storage systems, while backup to object uses a storage provider as the destination.
...
So if you're planning to create custom policies when you use the cascade architecture, you'll need to create the replication and backup-to-object policies on the secondary system where the replicated volumes will be created. If you're planning to create custom policies when you use the fan-out architecture, you'll need to create the replication policies on the secondary system where the replicated volumes will be created and the backup-to-object policies on the primary system.
...
NOTE: When creating a container to use as the backup destination in Azure, ensure that the container you create has the same name as the Azure storage account.
...
If you're already using the Console to manage your storage, a Console agent is already installed. If you plan to use the same Console agent with NetApp Backup and Recovery, you're all set. If you need to use a different Console agent, install it before you start your backup and recovery implementation.
...
When NetApp Backup and Recovery is used in a site with full internet connectivity (also known as _standard mode_ or _SaaS mode_), you can create replicated volumes on any on-premises ONTAP or Cloud Volumes ONTAP systems managed by the Console, and you can create backup files in object storage in any of the supported cloud providers. link:concept-backup-to-cloud.html[See the full list of supported backup destinations].
...
//* You can back up data from local on-premises ONTAP systems to local StorageGRID systems. link:prev-ontap-backup-onprem-storagegrid.html[Back up on-premises ONTAP data to StorageGRID].
//ifdef::aws[]
