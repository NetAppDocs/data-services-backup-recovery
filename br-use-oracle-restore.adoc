...
. Select *Next*.
+
All available snapshots are displayed. 
. Select a snapshot from the list and select *Next*.
+
Recovery is skipped and log backups are not applied.
. If the snapshot is stored in more than one location, select a source location to use and add or review location details. 
. Select *Next*.
--
====
...
* Control file restore cannot be combined with point-in-time recovery of PDBs or PDB tablespaces.
* Ensure the data backup and the required log backups are cataloged.
* The `SYSTEM`, `SYSAUX`, and `UNDO` tablespaces cannot be selected.
* Verify that the data backup and required log backups are cataloged.
...
. Specify external archive log file locations in the *External archive log files locations* field if restoring PDBs or tablespaces to a specific point in time or SCN, the archive logs are not in the configured location. Enter a file system path that starts with `/` or an ASM disk group that starts with `+`. Use the "plus" and "minus" buttons next to the text entry field to add or remove locations. This field does not appear when you restore from snapshots with no recovery.
