...
* Choose a timeout value for the script. If the script fails to execute within this time period, the restore proceeds anyway.
...
* *Open the database or container database in READ-WRITE mode after recovery*: After the restore operation is complete, Backup and Recovery enables READ-WRITE mode for the database.
