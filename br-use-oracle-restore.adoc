...
[NOTE]
=====
If restoring PDBs or PDB tablespaces to a specific point in time or SCN:

* Select only one PDB. Point-in-time recovery is not compatible with restoring multiple PDBs.
* PDB$SEED cannot be selected for point-in-time recovery.
* Control file restore cannot be combined with point-in-time recovery of PDBs or tablespaces.
* Ensure the data backup and the required log backups are cataloged.
=====
...
. Optionally, enable the *Control files* option to also restore the database control files. This option is not available for point-in-time or SCN recovery of PDBs or PDB tablespaces.
...
. Optionally, enable the *Force in place restore* option to use a faster restore method for SAN/ASM databases, skipping the foreign files validation check. The database and the ASM disk group are restored to the backup creation point.
. Specify the external archive log files location if restoring PDBs or PDB tablespaces to a specific point in time or SCN. Enter the location where RMAN creates the auxiliary instance to perform the recovery. Enter a file system path that starts with "/" or an ASM disk group that starts with "+". Use the "plus" and "minus" buttons next to the text entry field to add or remove locations.
. Optionally, enable the *Open the database or container database in READ-WRITE mode after recovery* to open the databases in READ-WRITE mode after restoring them. This brings the restored tablespaces online.
...
--
Not available for Oracle Database workloads in this preview release. 
