...
Note the following scenarios when using the private preview of KVM workloads with NetApp Backup and Recovery:

* You cannot restore KVM workload backups that were made with the *Enable VM-consistent snapshot* setting enabled in the protection policy. As a workaround, enable this setting when backing up KVM workloads.
* You cannot back up a storage pool with KVM hosts managed by Apache CloudStack unless you add all those hosts to NetApp Backup and Recovery. As a workaround, add every CloudStack-managed KVM host to NetApp Backup and Recovery.
