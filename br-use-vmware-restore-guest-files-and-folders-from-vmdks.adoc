...
You can restore to a remote VM (known as a cross-VM restore), but the backup source and restore destination operating systems must be the same.
...
. Select *Next*.
. Enter the share path to the guest where the selected files will be restored:
+
...
** IPv4 remote path: 10.60.136.65:/home/user/restore/
** IPv6 remote path: fd20-8b1e-b255-832e--61.ipv6-literal.net:/home/user/restore/
+
