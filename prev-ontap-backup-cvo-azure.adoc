...
= Back up Cloud Volumes ONTAP to Azure Blob storage with NetApp Backup and Recovery
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
[.lead]
Use NetApp Backup and Recovery to back up volume data from your Cloud Volumes ONTAP systems to Azure Blob storage.
...
You also need an Azure subscription for the storage space where your backups will be located.
...
To use the NetApp Backup and Recovery Search & Restore functionality, the role for the Console agent must include specific permissions so it can access the Azure Synapse workspace and Azure Data Lake Storage account. See the permissions below, and follow the steps if you need to modify the policy.

.Before you start

* You must register the Azure Synapse Analytics Resource Provider (called "Microsoft.Synapse") with your Subscription. https://learn.microsoft.com/en-us/azure/azure-resource-manager/management/resource-providers-and-types#register-resource-provider[See how to register this resource provider for your subscription^]. You must be the Subscription *Owner* or *Contributor* to register the resource provider.
...
See https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-deploying-otc-azure.html[Launching Cloud Volumes ONTAP in Azure^] for requirements and details for creating your Cloud Volumes ONTAP system.
...
+ 
NOTE: If you chose an existing Azure storage account, encryption information is already available, so you don't need to enter it now.
