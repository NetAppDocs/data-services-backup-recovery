...
[.lead]
Use these steps to enable preview and hidden features for NetApp Backup and Recovery. Use preview features with caution in production environments.
...
|`"data-transfer-report": true` |Enables the daily backup data transfer report under the Reports tab in NetApp Backup and Recovery for Volumes. The report provides daily backup and replication data transfer activity information across your environment.
|===
...
. Use SSH to log in to the Console agent.
...
----
. Create a file named `production-customer.json` and customize the storage VM and volume names to match your environment:
+
[source,json]
----
{
