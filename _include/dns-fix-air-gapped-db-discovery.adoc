...
. Copy the following command block and edit the `configBlock` area to add required static DNS records for your environment. The command creates a HelmChartConfig YAML file in the RKE2 manifests folder on the NetApp Console (local deployment) server node:
+
[source,shell]
----
