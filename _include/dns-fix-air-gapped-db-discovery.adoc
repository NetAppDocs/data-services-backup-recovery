...
.. Run the following script to append the `servers:` block to the existing HelmChartConfig file:
+
[source,shell]
----
...
.. After restoring the file, remove the host entries. Stale entries can cause host discovery and scheduled backup issues.
...
.. After deleting the file, remove the host entries. Stale entries can cause host discovery and scheduled backup issues.
