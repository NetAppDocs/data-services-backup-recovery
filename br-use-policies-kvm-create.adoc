...
= Create and manage KVM backup policies in NetApp Backup and Recovery
:hardbreaks:
:icons: font
:imagesdir: ./media/

[.lead]
Use NetApp Backup and Recovery to create KVM backup policies that control backup frequency, schedule, and retention.
...
====
* *VM Settings*:
** *Enable VM-consistent and application-consistent snapshot*: Enable this option to create VM-consistent and application-consistent snapshots. This requires the KVM QEMU Guest Agent to be running on the VM. If the Guest Agent is not running, snapshots are crash-consistent instead. Enabling this option can increase backup time and use more storage. Additionally, the VM's active RAM is not included in consistent snapshots.
