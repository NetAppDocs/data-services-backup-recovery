...
summary: You can use Search and Restore to recover volumes, folders, or files from ONTAP backup files.
---

= Restore ONTAP data using Search and Restore in NetApp Backup and Recovery
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
[.lead]
Use Search and Restore to recover volumes, folders, or files from ONTAP backups when you don't know the exact system, volume, or file name. You can search across local snapshots, replicated volumes, and object storage backups.
...
* When you restore a SnapLock-enabled volume, the restored volume retains SnapLock protection only if the destination storage is WORM-enabled. If you restore to a non-WORM destination, the SnapLock status is removed from the restored volume.
====
...
Before you begin, you should have some idea of the name or location of the volume or file you want to restore.
...
On-premises ONTAP system
//endif::gcp[]
| StorageGRID | On-premises ONTAP system
Cloud Volumes ONTAP
| On-premises ONTAP system
...
** The SnapDiff RPC Server must be activated on the storage VM. The Console does this automatically when you enable Indexing on the system. (SnapDiff is the technology that quickly identifies the file and directory differences between snapshots.)
...
+
NOTE: Backup and Recovery does not index any snapshots whose creation timestamp exactly matches another snapshot of the same volume.
...
First, mount a separate volume to hold catalog files. This helps prevent data loss if the snapshot catalog files become too large. This is not required on every cluster; you can mount any one volume from any of the clusters in your environment. If you don't do this, indexing might not function correctly.
...
[#mount-volume-steps]
.Step to mount the volume for catalog reindexing
...
.. In the _Filter by_ area, select the filter criteria. For example, you can select the system where the data resides and the file type, such as a .JPEG file. You can also select the backup location type if you want to search only within available snapshots or backup files in object storage.
...
+
Note that the results identify local volume snapshots and remote replicated volumes that contain the file in your search. You can choose to restore from the cloud backup file, the snapshot, or the replicated volume.
...
+
//ifdef::aws[]
** When restoring from Amazon S3, select the IPspace in the ONTAP cluster where the destination volume will reside, enter the access key and secret key for the user you created to give the ONTAP cluster access to the S3 bucket, and optionally choose a private VPC endpoint for secure data transfer. If you use a private VPC endpoint, it takes precedence over any configured proxy connection. link:prev-ontap-backup-onprem-aws.html[See details about these requirements].
//endif::aws[]
//ifdef::azure[]
