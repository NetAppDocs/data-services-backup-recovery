...
[.lead]
In NetApp Backup and Recovery, you can use the default backup policies or create custom policies for your ONTAP volumes. Policies control backup frequency, backup timing, and retention.
...
. In the Policies management page, select *Create policy* > *Create Snapshot policy*.
...
.Steps
. In the Policies management page, select *Create policy* > *Create replication policy*.
...
.Steps
. In the Policies management page, select *Create policy* > *Create backup policy*.
. In the Policy Details section, specify the policy name. 
. Specify the SnapMirror labels (maximum of 5) denoting the retention for each label.
