...
+
NOTE: If the database host is clustered, enter the Single Client Access Name (SCAN) in the *Host FQDN or IP address* field.
...
.. Choose whether to allow NetApp Backup and Recovery to install the plug-in automatically on each host or skip the automatic plug-in installation.
+
NOTE: Backup and Recovery uses SSH to connect to each host when installing the plug-in automatically. Enable *Use manual installation* when any of the following apply:
* One or more hosts do not have SSH access.
...
.. (Optional) if the database host is clustered, enable the *Add all hosts in the cluster* option to discover all hosts in the cluster. 
