---
sidebar: sidebar
permalink: br-use-hyperv-protection-groups.html
keywords: backing up, back up, backup, backup application data, Amazon Web services, AWS, NetApp Backup and Recovery, Hyper-V, vm, virtual machines, workloads, volumes, snapshots
summary: Create and manage protection groups for Hyper-V workloads in NetApp Backup and Recovery. 
---

= Create and manage Hyper-V protection groups in NetApp Backup and Recovery
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
:imagesdir: ./media/

[.lead]
Create protection groups to protect related Hyper-V VMs together and manage their backup operations in NetApp Backup and Recovery.

You can perform the following tasks related to protection groups:

* Create a protection group.
* View protection group details. 
* Back up a protection group now. See link:br-use-hyperv-backup.html[Back up Hyper-V workloads now].
//* Suspend and resume a protection group's backup schedule.  
* Delete a protection group.

== Create a protection group 

Group workloads that you want to protect together into a protection group. Create a protection group to back up and restore those workloads together.

*Required NetApp Console role*
Backup and Recovery super admin or Backup and Recovery backup admin role. https://docs.netapp.com/us-en/console-setup-admin/reference-iam-predefined-roles.html[Learn about NetApp Console access roles for all services^].

.Before you begin
* Be aware that for Hyper-V workloads, the scheduled backup time is interpreted in the
time zone of the management host. See link:br-use-policies-hyper-v-create.html[Create and manage Hyper-V backup policies in NetApp Backup and Recovery] for more information.

.Steps
. From the NetApp Backup and Recovery menu, select *Inventory*.
. Select a workload to view its protection details.
. Select the Actions icon image:../media/icon-action.png[Actions option] > *View details*.
. Select the *Protection groups* tab.
. Select *Create protection group*.
. Enter a name for the protection group.
. Select the VMs that you want to include in the protection group.
. Select *Next*.
. Select a behavior for spanning disks. Currently, *Always include all spanning disks* is supported.
+
NOTE: For CIFS configurations, Backup and Recovery automatically protects every disk attached to each VM in the protection group. This includes files distributed across multiple ONTAP volumes or ONTAP storage systems (spanning disks) and files limited to a single location (non-spanning disks). For SAN configurations, all non-spanning disks will be included in the protection group.

. Select *Next*.
. Select the protection policy that you want to apply to the protection group.
+
Optionally, you can select *Create new policy* to create a new protection policy. When you select a policy, policy details appear.
. When finished, select *Next*. 
. Review the configuration.
. Select *Create* to create the protection group.



////

== Suspend a protection group's backup schedule

Suspending a protection group pauses the scheduled backups for the protection group. You might want to suspend a protection group if you want to temporarily stop backups for the workloads in that group. 

The protection status changes to "Under maintenance" when you suspend a protection group. You can resume the backup schedule at any time.

.Steps
. From the NetApp Backup and Recovery menu, select *Inventory*.
//+
//image:screen-vm-inventory.png[Inventory screenshot for NetApp Backup and Recovery]
. Select a workload to view the protection details.
. Select the Actions icon image:../media/icon-action.png[Actions option] > *View details*.
. Select the *Protection groups* tab.
. Select the Actions icon image:../media/icon-action.png[Actions option] > *Suspend protection group*.

. Review the confirmation message and select *Suspend*. 


== Resume a protection group's backup schedule

Resuming a suspended protection group restarts the scheduled backups for the protection group.  

The protection status changes from "Under maintenance" when you suspend a protection group to "Protected" when you resume it. You can resume the backup schedule at any time.

.Steps
. From the NetApp Backup and Recovery menu, select *Inventory*.
//+
//image:screen-vm-inventory.png[Inventory screenshot for NetApp Backup and Recovery]
. Select a workload to view the protection details.
. Select the Actions icon image:../media/icon-action.png[Actions option] > *View details*.
. Select the *Protection groups* tab.
. Select the Actions icon image:../media/icon-action.png[Actions option] > *Resume protection group*.

. Review the confirmation message and select *Resume*. 

.Result
The system validates the schedules and changes the protection status to "Protected" if the schedules are valid. If the schedules are not valid, the system displays an error message and does not resume the protection group.  

////

== Edit a protection group

Edit a protection group to change its name or settings. You might want to edit a protection group if the resources in the group have changed.

.Steps
. From the NetApp Backup and Recovery menu, select *Inventory*.
. Select a workload to view the protection details.
. Select the Actions icon image:../media/icon-action.png[Actions option] > *View details*.
. Select the *Protection groups* tab.
. Select the protection group that you want to edit.
. Select the Actions icon image:../media/icon-action.png[Actions option] > *Edit*.
+
The Edit protection group wizard guides you through the settings in the protection group.
. Make any necessary changes on each screen of the wizard.
. When finished, select *Submit*.

=== Apply an updated host time zone to an existing protection group

Scheduled backups use the Hyper-V host's time zone that is recorded when the protection group's schedule is created. If you change the host's time zone, existing protection groups continue to run on their original time zone (typically UTC) until you update their schedules.

NOTE: Schedules are only re-created when you modify the protection group name, add or remove a VM, change the assigned policy, or update pre/post scripts. Saving without any changes will not update the schedules. This one-time schedule update does not start an immediate backup.

.Steps

. Refresh the Hyper-V host so its current time zone is retrieved.
. Edit the protection group and change its name to a different value, then save.

== Delete a protection group

Deleting a protection group removes it and all associated backup schedules. You might want to delete a protection group if it is no longer needed.

.Steps
. From the NetApp Backup and Recovery menu, select *Inventory*.
. Select a workload to view its protection details.
. Select the Actions icon image:../media/icon-action.png[Actions option] > *View details*.
. Select the *Protection groups* tab.
. Select the protection group that you want to delete.
. Select the Actions icon image:../media/icon-action.png[Actions option] > *Delete*.
. Review the confirmation message about deleting the associated backups and confirm the deletion.
