...
= Restore VMware virtual disks from backups in NetApp Backup and Recovery
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
[.lead]
Restore existing, deleted, or detached virtual disks (VMDKs) from primary or secondary backups of traditional VMs. This lets you recover specific VM data or applications without restoring the entire VM. After the virtual disk is restored, it is attached to its original VM and is ready to use.
...
However, if you back up VMDKs that are attached to a VM with an NVMe disk, attach and restore operations use an NVMe controller if one is available.
