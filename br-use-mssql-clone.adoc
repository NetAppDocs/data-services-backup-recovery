...
summary: Clone Microsoft SQL Server application data for development, testing, or protection purposes.
---
...
[.lead]
Create, refresh, split, and delete Microsoft SQL Server clones for development, testing, or protection.
...
You can create a clone from an existing snapshot or from an instant snapshot.
...
* *Source database host, instance, and name*: Select the source database host, instance, and name for the clone. The source database is the database from which the clone is created. 


. Complete the *Database target* section:

* *Target database host, instance, and name*: Select the target database host, instance, and name for the clone. The target database is the location where the clone is created. 
+
...
. If you choose to create *Instant snapshot and clone*, choose the clone storage location: 
...
. If you choose *Instant snapshot and clone*, complete the following options:
+
* *Clone refresh schedule and expiration*: Enter the date when to begin refreshing the clone. The refresh schedule defines when the clone refresh begins.
** *Delete clone if schedule expires*: Select this option to delete the clone when the schedule expires.
** *Refresh clone every*: Select the refresh frequency. You can choose to refresh the clone hourly, daily, weekly, monthly, or quarterly. This option keeps the clone synchronized with the source workload.
...
* *Notification*: Optionally, specify email addresses to receive notifications about the clone creation status along with the Job report. You can also specify a webhook URL to receive notifications about the clone creation status. You can choose to receive success notifications, failure notifications, or both. 
...
Skip the next scheduled refresh to keep the clone unchanged.
...
You can split a clone of your Microsoft SQL Server workloads to create an independent or long-term copy. Splitting a clone creates a new backup from the clone that can be used for protection and restore operations.
...
== Delete a clone
Remove clones of your Microsoft SQL Server workloads to free up storage space. If a policy protects the clone, both the clone and its associated job are deleted.
