...
Protection groups created before host-time-zone scheduling was introduced continue to use their previously configured time (typically UTC). To update these protection groups to use the Hyper-V host time zone, refresh the Hyper-V host and then edit the protection group (for example, by renaming it) to re-create its schedules with the new time zone.