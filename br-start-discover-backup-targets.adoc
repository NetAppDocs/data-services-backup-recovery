...
[.lead]
Discover or manually add offsite backup targets such as AWS S3, Azure Blob Storage, Google Cloud Storage, or StorageGRID so you can store backups outside your primary environment. 

== Discover a backup target 

Configure your backup targets (Amazon Web Services (AWS) S3, Microsoft Azure Blob Storage, Google Cloud Storage, or StorageGRID) before you use NetApp Backup and Recovery.

You can discover these targets automatically or manually add them.

Provide credentials to access the storage account. NetApp Backup and Recovery uses these credentials to discover and verify access to your backup target buckets.
...
.Steps
. From the NetApp Backup and Recovery menu, select *Backup targets*.
. Select *Discover backup target*. 
. Select one of the available backup target types: *AWS*, *Azure*, *StorageGRID*, *ONTAP S3*, or *GCP*.
. Select *Next*.
. (For AWS and Azure backup targets only) In the *Choose credentials location* section, choose the location where the credentials reside and then choose how to associate the credentials. 
. Select *Next*.
...
** *Application (client) ID*: Enter the Azure Blob Storage application ID.
** *Directory tenant ID*: Enter the Azure Blob Storage tenant ID.

* For StorageGRID:  
** *Credential name*: Enter the StorageGRID credential name.
** *Gateway Node FQDN*: Enter an FQDN for the StorageGRID gateway node.
...
** *Gateway Node FQDN*: Enter an FQDN for the ONTAP S3 gateway node.
** *Port*: Enter the port number for ONTAP S3.
** *Access key*: Enter the ONTAP S3 access key. 
** *Secret key*: Enter the ONTAP S3 secret key. 

* For GCP: 
** *Credentials name*: Enter the Google Cloud credential name.
//** *GCP project*: Enter the name of your Google Cloud project.
** *Access key*: Enter the Google Cloud access key. 
** *Secret key*: Enter the Google Cloud secret key. 

. Select *Discover*.

== Add a bucket for a backup target 

Manually add a bucket to an offsite backup target instead of having NetApp Backup and Recovery discover buckets automatically.

.Steps
. From the NetApp Backup and Recovery menu, select *Backup targets*.
. Select a target and on the right, select the *Actions* image:icon-action.png["Actions icon"] icon and select *Add bucket*.
...
* For Azure: 
** *Subscription*: Select the name of the Azure subscription.
** *Resource group*: Select the name of the Azure resource group.
** *Instance details*: 
*** *Storage account name*: Enter the name of the Azure Blob Storage account.
*** *Azure region*: Enter the Azure region for the storage account.
*** *Performance type*: Select the performance type of either standard or premium for the Azure Blob Storage account, indicating the level of performance required. 
*** *Encryption*: Select the type of encryption for the Azure Blob Storage account. The options are either Microsoft-managed keys or customer-managed keys. If you select customer-managed keys, you must provide the key vault name and key name.

* For StorageGRID: 
** *Backup target*: Select the name of the StorageGRID backup target.
...
* For ONTAP S3: 
** *Backup target*: Select the name of the ONTAP S3 backup target.
** *Bucket name*: Enter the name of the ONTAP S3 bucket.
...
** *GCP project*: Select the name of the Google Cloud project.
** *Bucket region*: Select a region for the Google Cloud bucket.
** *Object locking*: Select this option to enable retention periods for objects in this bucket. While a retention period is in effect, objects cannot be modified or deleted and are immutable until the period expires. If you enable this option for a bucket, it cannot be turned off. 
...
.Steps
. From the NetApp Backup and Recovery menu, select *Backup targets*.
. Select a target and on the right, select the *Actions* image:icon-action.png["Actions icon"] icon and select *Change credentials*.
