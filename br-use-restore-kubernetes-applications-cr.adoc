...
= Restore Kubernetes applications with custom resources
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
[.lead]
Use custom resources to restore your applications from a snapshot or backup. Restoring from an existing snapshot is typically faster when you restore the application to the same cluster.
...
* You can use advanced settings to customize restore operations. To learn more, see link:br-use-kubernetes-advanced-restore-settings.html[Advanced restore settings for Kubernetes in NetApp Backup and Recovery].
====
...
* When you use a CR to restore to a new namespace, you must manually create the destination namespace before applying the CR. Backup and Recovery automatically creates namespaces only when you use the CLI.
====
...
----
+
NOTE: If the BackupRestore CR is not available, you can use the command in step 2 to view the backup contents.
