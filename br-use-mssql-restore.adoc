...
summary: Restore database workloads from a snapshot, a workload backup replicated to secondary storage, or from backups stored in object storage.
---
...
[.lead]
Restore Microsoft SQL Server workloads from snapshots, backups replicated to secondary storage, or backups in object storage. Choose the original system, a different system, or an on-premises ONTAP system as the destination.

.Restore sources

You can restore workloads from the following locations:
...
Use the Restore page to restore database workloads.
...
*** *Operational*: Brings the database online after transaction log backups are applied. You cannot restore additional transaction logs.
*** *Non-operational*: Keeps the database unavailable so that you can restore additional transaction logs.
*** *Read-only mode*: Restores the database in read-only mode and lets you restore additional transaction logs.
...
This option uses the latest full and log backups to restore data to the last good state. NetApp Backup and Recovery scans logs from the last snapshot to the present to restore the most recent available version of the data. 
...
. On the *Restore to the latest state* page, select a snapshot location: local storage, secondary storage, or object storage.
. Select *Next*.
...
Use the Inventory page to restore database workloads. You can restore databases, but not instances, from this page. 
...
. Complete the steps for the selected restore option from the Restore page.
