...
= KVM protection workflow with NetApp Backup and Recovery
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
[.lead]
Protect your managed KVM VMs and storage pools with NetApp Backup and Recovery. The service provides fast, space-efficient, crash-consistent, and VM-consistent backup and restore operations.

Your KVM hosts and VMs must be managed by a platform such as Apache CloudStack before you can protect them with NetApp Backup and Recovery.
...
* Using different media types helps you recover if one type fails.
* You can quickly restore from the onsite copy, and use the offsite copies if the onsite copy is compromised.

You can use NetApp Backup and Recovery to perform the following tasks for KVM workloads:
