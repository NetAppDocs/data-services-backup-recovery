...
= Back up Cloud Volumes ONTAP to Amazon S3 with NetApp Backup and Recovery
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
[.lead]
Use NetApp Backup and Recovery to back up volume data from your Cloud Volumes ONTAP systems to Amazon S3.
...
NOTE: If you use a private endpoint (VPC endpoint) for backup or restore data transfers, it takes precedence over any configured proxy connection.
...
You also need an AWS account for the storage space where your backups will be located.
...
You can enable NetApp Backup and Recovery on a new or existing Cloud Volumes ONTAP system. The steps differ slightly for each option.
...
+
If the Amazon S3 destination for your backups exists as a system on the *Systems* page, you can drag the cluster onto the Amazon S3 system to initiate the setup wizard.


== Activate backups on your ONTAP volumes

Activate backups at any time directly from your Cloud Volumes ONTAP system.
...
+
If the Amazon S3 destination for your backups exists as a system on the Console *Systems* page, you can drag the ONTAP cluster onto the Amazon S3 system.
* Select *Volumes* in the Backup and Recovery bar. From the Volumes tab, select the *Actions* image:icon-action.png["Actions icon"] icon and select *Activate 3-2-1 Protection* for a single volume (that does not already have replication or backup to object storage enabled).

+ 
The Introduction page of the wizard shows the protection options, including local snapshots, replication, and backups. If you used the second option in this step, the Define Backup Strategy page appears with one volume selected.
...
+ 
If you choose to use your own customer-managed keys, enter the key information.
+ 
...
The Volume Backup Dashboard appears so you can monitor the state of the backups.
