...
+
* *RMAN catalog settings*: (Applies to Oracle Database workloads only) Catalog metadata in the Oracle Recovery Manager (RMAN) recovery catalog. The RMAN recovery catalog is a separate database schema used by RMAN to store metadata about backups and recovery operations for registered databases. It provides redundancy and long-term retention in case the target control file is lost or when historical data is needed. Enable the *Catalog backup with Oracle Recovery Manager (RMAN)* option to register datafiles and archivelog copies in the RMAN repository so that RMAN can track and manage those copies for reporting and recovery.
+
NOTE: If you enable this option and have selected *Target control file* for the *Configure RMAN catalog settings* section in the *Configure database* area, Backup and Recovery writes RMAN metadata to the target control file.
