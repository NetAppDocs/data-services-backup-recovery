...
Optionally, you can configure advanced settings in the policy. These settings are available for all backup architectures, including local snapshots, replication to secondary storage, and backups to object storage. The available advanced settings will differ depending on the workload you selected at the top of the page, so the advanced settings described here might not apply to all workloads. Advanced settings are not available when configuring a policy for Kubernetes workloads.
