...
[.lead]
In NetApp Backup and Recovery, you can back up volume data from your on-premises ONTAP systems to a secondary storage system and to Amazon S3. Use this page to prepare your environment and activate backups.
...
NOTE: Customer-managed key encryption is supported only for Amazon FSx for NetApp ONTAP. It is not supported for Cloud Volumes ONTAP for AWS.

Refer to https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-setting-up-kms.html[Set up customer-managed key encryption in AWS^].
...
* *Bucket*: Either choose an existing S3 bucket or create a new one. Refer to https://docs.netapp.com/us-en/storage-management-s3-storage/task-add-s3-bucket.html[Add S3 buckets^].
