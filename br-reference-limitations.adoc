...
[.lead]
Review these limitations before using NetApp Backup and Recovery for ONTAP volumes.
...
* You can select only one FlexGroup volume at a time for replication. You need to activate backups separately for each FlexGroup volume.
+
FlexVol volumes have no such limitation: you can select all FlexVol volumes in your system and assign the same backup policies.
...
* When you convert a FlexVol volume to a FlexGroup volume or vice versa, the converted volume is considered a new volume; you need to set up protection for the converted volume after conversion. You can restore backups created before the conversion to a different volume.
...
* Do not back up SnapMirror destination volumes that have long-term retention snapshots enabled. These snapshots are created directly on the destination and will not be moved to object storage. This restriction applies even if the SnapMirror policy is later changed to disable long-term retention.
...
** Relationships with the SnapMirror labels `app_consistent` and `all_source_snapshot` will not be backed up to the cloud.
** If you create local copies of snapshots on the SnapMirror destination volume (irrespective of the SnapMirror labels used), these snapshots will not be moved to the cloud as backups. You must create a snapshot policy with the desired labels on the source DP volume so that NetApp Backup and Recovery can back them up.

* FlexGroup volume backups cannot be moved to archival storage.
...
* Ad-hoc volume backup using the *Backup Now* button is not supported on data protection volumes.

* SM-BC configurations are not supported.

* ONTAP does not support fan-out of SnapMirror relationships from a single volume to multiple object stores; therefore, this configuration is not supported by NetApp Backup and Recovery.

* WORM/Compliance mode on an object store is supported on Amazon S3, Google Cloud Platform (GCP), Azure, and StorageGRID at this time. This is known as the DataLock feature, and it must be managed by using NetApp Backup and Recovery settings, not by using the cloud provider interface.
...
* Restoring folders from local snapshots is not supported.

* Restoring files and folders in a disaster recovery (DR) configuration: After a failover, volumes on a storage VM configured for DR retain pre-existing SnapMirror relationships in a broken-off state. Attempts to restore files or folders that create temporary SnapMirror RST relationships are blocked by ONTAP because the destination volume already has a relationship.

* Browse & Restore can restore up to 100 individual files at a time.

* Search & Restore can restore one file at a time.
...
When enabling indexing on a storage VM cluster in a disaster recovery (DR) configuration, an error appears stating the operation is not permitted on a Vserver that is configured as the destination for Vserver DR. Despite the error, the backend process sets the `snapdiff-rpc-server` status to "on" for the source storage VMs. After the storage VM DR policy schedule runs, this status propagates to the destination storage VMs.

*Workaround*: Wait for the storage VM DR policy schedule to run after the initial error, then try enabling indexing again.