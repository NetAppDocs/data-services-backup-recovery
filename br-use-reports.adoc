...
= Create Microsoft SQL Server protection reports in NetApp Backup and Recovery
:hardbreaks:
:icons: font
:imagesdir: ./media/

[.lead]
Create reports for Microsoft SQL Server workloads in NetApp Backup and Recovery to review backup status and details, including counts of successful and failed backups, backup types, storage systems, and timestamps.

== Create a protection report

*Required NetApp Console role*
Backup and Recovery super admin, Backup and Recovery backup admin, Backup and Recovery restore admin, Backup and Recovery clone admin. If you are protecting data stored on a Cloud Volumes ONTAP cluster, the Cloud Volumes ONTAP Admin role is also required. Learn about link:reference-roles.html[Backup and recovery roles and privileges]. https://docs.netapp.com/us-en/console-setup-admin/reference-iam-predefined-roles.html[Learn about NetApp Console access roles for all services^]. 
...
. Enter the reporting range: Choose whether you want the report to include data from the last day, last 7 days, last 30 days, last quarter, or last year.
. Enter report delivery details: If you want the report to be delivered by email, select *Send report using email*. Enter the email address where you want the report sent.
+
