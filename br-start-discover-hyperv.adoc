...
... Select *Done*.
.. *Hyper-V settings*: Add a new Hyper-V host by entering the host's FQDN or IP address and selecting a Console agent and credentials. If the FQDN is not resolvable by the Console agent, use the IP address instead. For failover clusters, enter the cluster management IP address.
.. *Advanced settings*: Optionally, enter a port to use for communication with the NetApp Hyper-V plugin for Hyper-V and an installation path for the plugin. 
. Select *Discover*. 
+
