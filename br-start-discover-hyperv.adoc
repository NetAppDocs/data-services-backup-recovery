...
... Select *Done*.
.. *Host registration*: Add a new Hyper-V host by entering the host's FQDN or IP address, credentials, Console agent, and port number. If the FQDN is not resolvable by the Console agent, use the IP address instead. For failover clusters, enter the cluster management IP address.
. Select *Discover*. 
+
