...
+
* *Other Backup and Recovery roles*: Assign users the specific Backup and Recovery roles they need, such as Backup and Recovery viewer or Backup and Recovery clone admin. These roles allow users to perform day-to-day tasks such as viewing resources, protecting workloads, cloning, and restoring. Assign Backup and Recovery roles separately from platform-level roles. For more information, refer to link:https://docs.netapp.com/us-en/console-setup-admin/reference-iam-backup-rec-roles.html[NetApp Backup and Recovery roles in NetApp Console^].
...
+
Organization admins or folder or project admins discover the available workloads and select the appropriate project (such as US East or US West). Each workload is automatically associated with the selected project. For Kubernetes clusters, you can optionally select a folder or project to associate with the cluster during discovery.
...
* link:br-use-manage-kubernetes-access-control.html[Manage access to Kubernetes resources]