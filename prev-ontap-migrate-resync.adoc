...
[.lead]
Use SnapMirror to Cloud Resync in NetApp Backup and Recovery to keep existing cloud backups available after you migrate a volume with SnapMirror Logical Replication (LRSE). This feature lets backups continue without a re-baseline and is available starting with ONTAP 9.16.1.

SnapMirror to Cloud Resync supports FlexVol and FlexGroup migrations and helps you maintain backup continuity across hybrid and multi-cloud environments.
...
* Ensure that the latest backup in the object storage is the common snapshot across the old source, the new source, and the object store. Do not use a common snapshot that is older than the latest snapshot backed up to the object store. 
...
Imagine you have an on-premises volume called Vol1a. This volume has three snapshots: S1, S2, and S3. These snapshots are restore points. Vol1a is backed up to the cloud using SnapMirror to Cloud (SM-C), but only S1 and S2 are in the object store.
...
NOTE: Do not use a SnapMirror policy of type "Vault" to clone a volume from an old ONTAP cluster to a new ONTAP cluster or in a cascading architecture setup. Vault policies change the version UUID of the snapshot during the transfer. The version UUID of the snapshot is used to determine whether a common snapshot exists between the source and destination.
...
+
NetApp Backup and Recovery validates that each volume selected for resync shares a common snapshot with the object store. Specifically, the latest snapshot stored in the object store must also exist in the local snapshot of the new cloned volume. This check ensures the volumes are ready for the SnapMirror to Cloud Resync operation.

. Review validation results including the new source volume names and the resync status for each volume. 

. Check volume eligibility. The system checks if the volumes are eligible for resync. If a volume is not eligible, it means that the common snapshot criteria were not met.
