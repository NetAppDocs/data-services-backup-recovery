...
NOTE: A SnapMirror policy of type "Vault" must not be used to clone a volume from an old ONTAP cluster to a new ONTAP cluster, nor should it be used in a cascading architecture setup. The reason is that Vault policies change the version UUID of the snapshot during the transfer. The version UUID of the snapshot is used to determine whether a common snapshot exists between the source and destination.
...
. Check volume eligibility. The system checks if the volumes are eligible for resync. If a volume is not eligible, it means that the common snapshot criteria was not established. 
