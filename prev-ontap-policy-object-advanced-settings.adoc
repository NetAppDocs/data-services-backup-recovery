...
= Manage backup-to-object advanced settings in NetApp Backup and Recovery
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
[.lead]
Use the Advanced Settings page to change cluster-level backup-to-object storage settings for each ONTAP system after you activate NetApp Backup and Recovery. You can adjust bandwidth, historical snapshot export, yearly snapshot deletion, and ransomware scan settings.
...
//* If you are using a different IPspace than the _Default_, you might need to create a static route to get access to the object storage.
//
...
. From the _Backup Settings page_, select image:icon-actions-horizontal.gif[More icon] for the system and select *Configure advanced settings > System settings*.
...
Ransomware protection scans are enabled by default. By default, scans run every 7 days. The scan occurs only on the latest snapshot.
...
You can change that schedule to a number of days or weeks, or disable it to save costs.
...
. From the _Backup Settings page_, select image:icon-actions-horizontal.gif[More icon] for the system and select *Configure advanced settings > Provider settings*.
