...
Ransomware protection scans are enabled by default. The default setting for the scan frequency is for 7 days. The scan occurs only on the latest snapshot. You can enable or disable ransomware scans on the latest snapshot by using the option on the Advanced Settings page. If you enable it, scans are performed every 7 days by default. 
...
Scheduled ransomware scans run only on the latest snapshot.
