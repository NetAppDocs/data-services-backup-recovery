---
sidebar: sidebar
permalink: vmware-restore-guest-files-and-folders-from-vmdks.html
keywords:
...
By default, the system makes the attached virtual disk available for 24 hours, then automatically detaches it. In the wizard, you can choose to delete the guest file restore session automatically after the restore operation or delete the session manually.
...
[IMPORTANT]
The guest restore feature allows you to view and restore system and hidden files and to view encrypted files. Do not overwrite an existing system file or restore encrypted files to an encrypted folder. During the restore operation, the hidden, system, and encrypted attributes of guest files are not kept in the restored file. Viewing or browsing reserved partitions might cause an error.
...
+
Select this option to receive an email when the attach operation finishes and the virtual disk is available. NetApp Backup and Recovery sends an email that includes the virtual disk name, VM name, and new drive letter for the VMDK.
+
[TIP]
...
+
NetApp Backup and Recovery assigns a drive letter to all partitions, including system reserved partitions. If a VMDK has multiple partitions, you can select a specific drive by selecting the drive in the drop-down list in the drive field at the top of the Guest File Browse page.

. Select the *Browse Files* icon to view a list of files and folders on the virtual disk.
+
When you choose a folder to browse and select files, there might be a delay as the system fetches the file list.
+
...
[NOTE]
If the folder already exists, the contents are added to the existing folder.
...
. Select *Restore*.
+
View the restore progress in the Dashboard job monitor or wait for the email notification, which is sent when the restore operation finishes.
+
