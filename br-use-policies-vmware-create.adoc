...
[.lead]
Use NetApp Backup and Recovery to create VMware backup policies that control backup frequency, schedule, and retention.
...
NOTE: For VMware workloads, backup schedules use the time zone of the VMware vCenter Server, not UTC or your browser's local time. This time zone is set when you create the schedule. If the host time zone changes, you must refresh the host and re-create the schedule for the new time zone to take effect. Refer to link:br-use-vmware-protection-groups.html[Create and manage protection groups for VMware workloads] for more information.
...
+
For VMware workloads, this configures the local snapshot on the datastores or VMs on the primary storage system and then replicates the data from the primary disk storage system to the secondary disk storage system.
...
====
* *VM Settings*:
** *Enable VM-consistent and application-consistent snapshot*: Enable this option to create VM-consistent and application-consistent snapshots. This requires VMware Tools to be running on the VM. If VMware Tools is not running, snapshots are crash-consistent instead. Note that enabling this option can increase backup time and use more storage. Additionally, the VM's active RAM is not included in consistent snapshots.
