...
NetApp Backup and Recovery leverages NetApp's SnapMirror data replication technology to ensure that all the backups are fully synchronized by creating Snapshot copies and transferring them to the backup locations.
...
* When compared to https://docs.netapp.com/us-en/bluexp-replication/index.html[NetApp  Replication^], the replication in NetApp Backup and Recovery includes the following features:
...
NetApp Backup and Recovery enables you to back up ONTAP volumes from the following source systems to the following secondary systems and object storage in public and private cloud providers. Snapshot copies reside on the source system.
...
* _Snapshot copies_ reside on the source volume in the source system.
...
When you enable NetApp Backup and Recovery for a system, all the volumes you initially select are backed up using the policies that you select. You can select separate policies for Snapshot copies, replicated volumes, and backup files. If you want to assign different backup policies to certain volumes that have different recovery point objectives (RPO), you can create additional policies for that cluster and assign those policies to the other volumes after NetApp Backup and Recovery is activated.
