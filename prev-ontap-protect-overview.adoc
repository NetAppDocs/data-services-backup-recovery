...
= Learn about ONTAP volume protection with NetApp Backup and Recovery
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
[.lead]
Use NetApp Backup and Recovery to protect and restore your ONTAP volume data and to archive backups for the long term. You can implement a 3-2-1 strategy with three copies of your source data on two different storage systems and one copy in the cloud.
...
NetApp Backup and Recovery uses NetApp's SnapMirror data replication technology to keep your backups synchronized by creating snapshots and transferring them to the backup locations.
...
* Set up native ONTAP in-flight encryption by using a pre-shared key (PSK) between the two systems.
...
* When compared to https://docs.netapp.com/us-en/data-services-replication/index.html[NetApp Replication^], replication in NetApp Backup and Recovery includes the following features:
...
* Scan backup files for possible ransomware attacks and automatically remove or replace infected backups.
...
NOTE: Wasabi is the only qualified S3-compatible object storage provider at this time. Other S3-compatible providers have not been validated.
...
NOTE: References to "on-premises ONTAP system" include FAS, AFF, and ONTAP Select systems.
...
Support is currently available when your backup destination is Amazon S3, Azure Blob, or StorageGRID. Other storage provider destinations will be added in future releases.
...
+
If your on-premises cluster is using ONTAP 9.12.1 or greater, and your StorageGRID system is using 11.4 or greater, you can archive older backup files to public cloud archival storage after a certain number of days. Current support is for AWS S3 Glacier/S3 Glacier Deep Archive or Azure Archive storage tiers. link:prev-ontap-backup-onprem-storagegrid.html[Learn more about archiving backup files from StorageGRID].

See link:prev-ontap-policy-object-options.html[how archiving older backup files works].
