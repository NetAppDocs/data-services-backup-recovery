...
[.lead]
Use this information to choose an AWS S3 archive tier for NetApp Backup and Recovery backups. You can also review restore costs and retrieval times for archived backups.

== Supported S3 archival storage classes for NetApp Backup and Recovery

When backup files are initially created, they're stored in S3 _Standard_ storage. This tier is optimized for storing data that's infrequently accessed, but it still allows immediate access. After 30 days, the backups transition to the S3 _Standard-Infrequent Access_ storage class to save on costs.
