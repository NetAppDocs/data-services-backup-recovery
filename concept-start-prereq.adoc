...
You access NetApp Backup and Recovery through NetApp Console. Perform these tasks to prepare the Console for Backup and Recovery, depending on whether you are using NetApp Console or NetApp Console local deployment.

=== Prepare to use Backup and Recovery on NetApp Console 
To prepare NetApp Console for Backup and Recovery, you must:
...
https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-standard-mode.html[Getting started workflow for NetApp Console^]
...
You can enable Backup and Recovery on a system when you add it, or at a later time.

link:br-start-enable.html#enable-backup-and-recovery-in-netapp-console[Enable Backup and Recovery for a system^].

==== Configure an organization and user roles
Learn about organizations, projects, fleets, and user roles, then configure users with the appropriate permissions needed to perform their tasks.

Refer to https://docs.netapp.com/us-en/console-setup-admin/task-iam-manage-members-permissions.html[Add users to a NetApp Console organization] for information about controlling resource access.
...
To prepare to use NetApp Backup and Recovery on NetApp Console local deployment, you must:

* Complete initial NetApp Console local deployment setup
* Sign up for preview enrollment
* Add at least one storage system
* Enable Backup and Recovery
* Grant access to resources for users

You need the Organization admin or Super admin role in NetApp Console local deployment to complete this preparation. 

To learn more about NetApp Console local deployment, refer to https://docs.netapp.com/us-en/console-local/concept-netapp-console-local.html[Learn about NetApp Console local deployment^].

==== Complete initial NetApp Console local deployment setup
Complete the initial setup for NetApp Console local deployment if you haven't already. Completing initial setup includes adding licenses for data services (including Backup and Recovery).

https://docs.netapp.com/us-en/console-local/task-quick-start-console-setup.html[Getting started with NetApp Console local deployment^]

==== Sign up for NetApp Backup and Recovery preview enrollment
Sign up for NetApp Backup and Recovery preview on NetApp Console local deployment:

https://forms.cloud.microsoft/pages/responsepage.aspx?id=oBEJS5uSFUeUS8A3RRZbOmNJwiQYiM5EhCH6-tDLH1hUMzBMVUY4Vk5DNDlRRzlKV1dORUlKQkUwNS4u&route=shorturl[Fill out the preview enrollment form^]

==== Add a storage system
Before you can protect workloads in Backup and Recovery, you need to add an ONTAP storage system to the Console. 

https://docs.netapp.com/us-en/console-local/task-discover-storage.html[Add storage in NetApp Console local deployment^]

==== Enable Backup and Recovery in NetApp Console local deployment
You can enable Backup and Recovery on a system when you add it, or at a later time.

link:br-start-enable.html#enable-backup-and-recovery-in-netapp-console-local-deployment[Enable Backup and Recovery for a system^].

==== Grant access to resources for users
Learn about identity and access management, then configure users with the appropriate permissions needed to perform their tasks.

Refer to https://docs.netapp.com/us-en/console-local/task-iam-fleet-mgmt-get-started.html[Get started with identity and access in NetApp Console local deployment] for information about identity and access.

Refer to https://docs.netapp.com/us-en/console-setup-admin/reference-iam-backup-rec-roles.html[NetApp Backup and Recovery roles in NetApp Console] for details about the roles and permissions needed to perform operations in NetApp Backup and Recovery.

== Requirements for backups to object storage

To use object storage as backup targets, you need an account with AWS S3, Google Cloud Storage, Azure Blob storage, StorageGRID, or ONTAP and the appropriate access permissions configured.
...
* ONTAP 9.19.1 or later is required to support more than 6 buckets per system across different cloud providers (up to 100 buckets maximum).
...
NOTE: ONTAP 9.19.1 or later is required to support more than 6 buckets per system across different cloud providers (up to 100 buckets maximum).
...
| Microsoft SQL Server versions | Versions 2012 and later are supported for VMware Virtual Machine File System (VMFS) and VMware Virtual Machine Disk (VMDK) on NFS.
...
|Connected storage
a|
* ONTAP 9.13.1 or later (ONTAP 9.19.1 or later is required to support more than 6 buckets per system across different cloud providers; up to 100 buckets maximum)
...
. Use the following command to add a non-root user to the `qemu` group while preserving their existing groups:
+
[source,shell]
----
...
* NetApp Trident 25.06 to 26.06
* NetApp Trident Protect 26.08 or later (installed on the Kubernetes cluster during discovery)
...
* To restore files and folders from VM-based applications, SSH is required. Verify that the SSH service is enabled on Linux destination hosts and that an OpenSSH server is installed and enabled on Windows destination hosts.
...
* Standalone hosts and failover cluster configurations are supported
...
* A primary ONTAP system (ONTAP 9.14.1 or later - ONTAP 9.19.1 or later is required to support more than 6 buckets per system across different cloud providers; up to 100 buckets maximum)
