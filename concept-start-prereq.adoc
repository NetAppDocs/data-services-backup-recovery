...
| Physical server
| FC-connected LUNs, iSCSI-connected LUNs

| VMware VM | iSCSI LUNs connected directly to the guest system by the iSCSI initiator, Virtual Machine File Systems (VMFS) or NFS datastores

| Hyper-V VM | iSCSI LUNs connected directly to the guest system by the iSCSI initiator. (Hyper-V pass through disks and backing up databases on VHD(x) that are provisioned on NetApp storage are not supported.)
