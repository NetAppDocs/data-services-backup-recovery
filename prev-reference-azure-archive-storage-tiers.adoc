...
[.lead]
Use this information to understand the Azure archive tier that NetApp Backup and Recovery supports. You can also review restore costs and retrieval times for archived backups.

== Supported Azure Blob access tiers for NetApp Backup and Recovery

When backup files are initially created, they're stored in the _Cool_ access tier. This tier is optimized for storing data that's infrequently accessed, but it can be accessed immediately when needed.
...
NOTE: High priority is not supported when restoring data from Azure to StorageGRID systems.
