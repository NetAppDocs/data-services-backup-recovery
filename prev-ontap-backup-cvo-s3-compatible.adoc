...
= Back up Cloud Volumes ONTAP data to S3-compatible storage with NetApp Backup and Recovery
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
[.lead]
Complete a few steps in NetApp Backup and Recovery to get started backing up volume data from your Cloud Volumes ONTAP systems to S3.
...
You can choose your own customer-managed keys for data encryption in the activation wizard instead of using the default S3 encryption keys. In this case you'll need to have the encryption managed keys already set up. https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-setting-up-kms.html[See how to use your own keys^].
...
+
If the S3-compatible destination for your backups exists as a cluster on the *Systems* page, you can drag the cluster onto the S3-compatible system to initiate the setup wizard.
...
* Select *Volumes* in the Backup and Recovery bar. From the Volumes tab, select the *Actions* image:icon-action.png["Actions icon"] icon option and select *Activate 3-2-1 Protection* for a single volume (that does not already have replication or backup to object storage already enabled).  
...
. *Backup*: Set the following options: 
+
...
. Optionally check the box to *Automatically fix mismatched labels on local Snapshot, replication and backup*. This creates snapshots with a label that matches the labels in the snapshot, replication and backup policies.  
