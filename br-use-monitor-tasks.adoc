...
A backup lifecycle job tracks all snapshots that are deleted or in the queue to be deleted. Beginning with ONTAP 9.13, you can look at all job types called "Retention" on the Job Monitoring page.
...
| Activation |Volume will be patched with Snapshot policy | Error | Yes
| Activation |Modify volume backup or state | Error | Yes
| Activation |Backup activated successfully for working environment | Error | Yes
| Activation |Ad-hoc volume backup failed | Error | Yes
| Activation |Modifying panoramic protection volumes failures | Error | Yes
| Cron operations |Checking for missing snapshot labels | Error | Yes
| Cron operations |Failed to send security token to ONTAP for this working environment | Error | Yes
| Pub/Sub events |Connection failure | Error | No
| Pub/Sub events |Failed to delete a scheduled snapshot | Error | No
| Pub/Sub events |Scheduled backup of volume failed | Error | No
| Pub/Sub events |Restore of volume succeeded  | Error | No
| Pub/Sub events |Restore of volume failed | Error | No
| Ransomware |Potential Ransomware attack identified on backup copy | Error | Yes
| Ransomware |Potential Ransomware attack identified on backup copy for system | Error | Yes
...
| Replication | Modification of replication relationship of volume failure | Error | Yes
...
| Target operations | Restore to local or cloud destination failure | Error | Yes
| Target operations | On-demand restore failure | Error | Yes
| System operations | Ad-hoc volume snapshot creation failure | Error | Yes
