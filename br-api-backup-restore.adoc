...
. Obtain a user token from the NetApp Console web site.
+
Make sure you generate the refresh token from the following link: https://services.cloud.netapp.com/refresh-token/. The refresh token is an alphanumeric string that you'll use to generate a user token.
+
[source,console]
----
