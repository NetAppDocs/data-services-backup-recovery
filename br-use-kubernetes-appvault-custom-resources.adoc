---
sidebar: sidebar
permalink: br-use-kubernetes-appvault-custom-resources.html
...
---
= Create AppVault objects for compatibility with Trident Protect backups
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
[.lead]
The bucket custom resource (CR) for Trident Protect is known as an AppVault. AppVault objects are the declarative Kubernetes workflow representation of a storage bucket. An AppVault CR contains the configurations necessary for a bucket to be used when restoring backups created using Trident Protect. Only administrators can create AppVaults.

You need to create an AppVault CR manually or from the command line when you restore backups created with Trident Protect. The AppVault CR is specific to your environment, and you can use the examples on this page as a guide when creating AppVault CRs.

NOTE: Ensure that the cluster where you create the AppVault CR also has Trident Protect installed. 
